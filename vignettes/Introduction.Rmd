...
This package, [cosinoRmixedeffects2](https://github.com/altintasali/cosinoRmixedeffects2), is forked from [cosinoRmixedeffects](https://github.com/maytesuarezfarinas/cosinoRmixedeffects). I have updated some parts of the vignette to build it properly while sticking by the original vignette.

Since [cosinoRmixedeffects2](https://github.com/altintasali/cosinoRmixedeffects2) is built according to [CRAN policies](https://cran.r-project.org/web/packages/policies.html), the parallel threads (`ncpus`) are set to 2. While you are running your own analysis, **I recommend setting `ncpus = 8`** or more considering that you have a modern computer.
...
```{r install the package, eval=FALSE, echo=TRUE}
remotes::install_github("altintasali/cosinoRmixedeffects2")
...
```{r message=FALSE}
db.means<-get.means.ci.cosinor(f1, contrast.frm="~gender", nsim=500, ncpus=2)
db.means
...
db.delta<-get.contrasts.ci.cosinor(f1,contrast.frm="~gender",nsim=500,ncpus=2)
db.delta
...
p<-cosinoRmixedeffects2::ggplot.cosinor.lmer(object=f1,
                       x_str="gender",
                       period=24,
                       db.means=db.means,
                       DATA=db.model)
...
```{r, message=FALSE}
db.means<-get.means.ci.cosinor(fit=f2, contrast.frm="~T0toT14", nsim=500, ncpus=2)
db.means
```
...
```{r, message=FALSE}
db.means2<-get.means.ci.cosinor(fit=f2, contrast.frm="~gender", nsim=500, ncpus=2)
db.means2
```
...
```{r, message=FALSE, fig.show='hide'}
db.delta<-get.contrasts.ci.cosinor(fit=f2,contrast.frm="~T0toT14", nsim=500, ncpus=2)
db.delta
```
...
```{r, message=FALSE}
db.means<-get.means.ci.cosinor(fit=f3, contrast.frm="~bmi_baseline_cat|gender", nsim=500, ncpus=2)
head(db.means)
```
...
```{r, message=FALSE, fig.show='hide'}
db.delta<-get.contrasts.ci.cosinor(fit=f3,contrast.frm="~bmi_baseline_cat|gender", nsim=500, ncpus=2)
head(db.delta)
```
